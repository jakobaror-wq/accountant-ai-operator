...
2. **Latency מול xAI** - כל צעד שולח תמונה ומחכה לתשובה; משימות ארוכות (עד 40 צעדים) יכולות לקחת זמן משמעותי. יש retry אוטומטי על תקלות חולפות (`task-runner.ts`). **עדכון:** נבנה מנגנון caching/מאקרו (`macros.ts`) שמדלג על קריאת ה-AI כשמשימה חוזרת על עצמה ומסך תואם למה שנצפה בהקלטה קודמת - ר' `05-DATA-MODEL.md` §4א ו-`09-COMPUTER-USE-AGENT.md`. לא אומת בפועל מול תוכנה אמיתית. **עדכון (2026-09-24)**: עד כה לקריאת ה-AI (`net.fetch` ב-`ai/grok.ts`) לא היה שום timeout - בקשה שנתקעת (פרוקסי/רשת) הייתה חוסמת את הלולאה **לצמיתות**, בלי שגיאה גלויה; זה תוקן (`AbortController`, 45 שניות) - ר' `09-COMPUTER-USE-AGENT.md`.
