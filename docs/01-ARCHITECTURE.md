...
window-focus.ts.focusConnectorWindow(connectorId) - מוצא/פותח/מביא לקדמת
   ...
   API; זיהוי לפי מיקום/גודל של "חלון חדש" שהופיע - לא לפי כותרת, ר' §6
   הערה) - אם נכשל, מוחזר started:false עם
...
main.ts - מציג חלון-חיווי תמיד-עליון ("🤖 הסוכן פעיל"), קורא ל-task-runner.ts.
   מ-2026-09-24 חלון-החיווי מתעדכן חי בכל שלב (step-start/action/awaiting-
   approval/awaiting-answer/error, ר' §6 הערה 2) - לא רק הודעה קבועה.
   ▼
...
**עדכון (2026-09-24):** בתחילת כל משימה, `window-focus.ts` מוצא/פותח/מביא לקדמת הבמה את חלון התוכנה בפועל. קודם לכן `aiop:run-task` פשוט התחיל לצלם את המסך בלי לוודא בכלל שהתוכנה הנכונה על המסך - ר' `09-COMPUTER-USE-AGENT.md` לפירוט התקלה שהובילה לתיקון הזה. **עדכון נוסף (עוד באותו יום):** הזיהוי המקורי היה לפי טבלת רמזי-כותרת סטטית (`apps/desktop/src/window-focus.ts`, לא `lib/connectors.ts` - שתי חבילות נפרדות) - בבדיקה חיה התגלה ש-nut-js מחזיר כותרות עבריות משובשות (באג קידוד, לא ניחוש שגוי), אז הזיהוי הוחלף לזיהוי "חלון חדש" לפי מיקום/גודל, בלי תלות בקריאת כותרת בכלל - ר' `09-COMPUTER-USE-AGENT.md` לפירוט.
...
2. **Latency מול xAI** - כל צעד שולח תמונה ומחכה לתשובה; משימות ארוכות (עד 40 צעדים) יכולות לקחת זמן משמעותי. יש retry אוטומטי על תקלות חולפות (`task-runner.ts`). **עדכון:** נבנה מנגנון caching/מאקרו (`macros.ts`) שמדלג על קריאת ה-AI כשמשימה חוזרת על עצמה ומסך תואם למה שנצפה בהקלטה קודמת - ר' `05-DATA-MODEL.md` §4א ו-`09-COMPUTER-USE-AGENT.md`. לא אומת בפועל מול תוכנה אמיתית. **עדכון (2026-09-24)**: עד כה לקריאת ה-AI (`net.fetch` ב-`ai/grok.ts`) לא היה שום timeout - בקשה שנתקעת (פרוקסי/רשת) הייתה חוסמת את הלולאה **לצמיתות**, בלי שגיאה גלויה; זה תוקן (`AbortController`, 45 שניות) - ר' `09-COMPUTER-USE-AGENT.md`. **עדכון (2026-09-26)**: אצל משתמש אמיתי, ה-timeout הזה נצפה בכל קריאה - אובחן כחשד חזק לפרוקסי ארגוני עם אימות (NTLM/Kerberos) ש-Electron, בניגוד לדפדפן, לא עובר אוטומטית בלעדיו; תוקן ב-`main.ts` (`auth-server-whitelist`) - לא אומת בפועל אם זה פתר את הבעיה, ר' `09-COMPUTER-USE-AGENT.md`.
