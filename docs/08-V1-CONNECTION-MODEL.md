...
לפני בניית שכבת Computer Use מלאה (UIA/Playwright/OCR, ר' `01-ARCHITECTURE.md`), הגרסה הראשונה היא **מסך חיבורים** - `apps/web` מסך `/integrations`, **בלי התחברות** (ר' עדכון v1.2 למטה), ונתוני החיבור נשמרים במשרד קבוע ויחיד (`connector_configs`, ר' `05-DATA-MODEL.md`). לכל אחת מ-4 התוכנות המשרד בוחר את סוג החיבור:
...
### עדכון v1.2 - הוסרה דרישת ההתחברות

v1.1 הוסיפה Supabase Auth (magic link) כדי לשייך כל משרד לחשבון משלו. בפועל זה יצר חיכוך גבוה מדי כרגע: שירות המייל המשותף של Supabase מוגבל למספר מיילים נמוך מאוד לשעה (מיועד לבדיקות בלבד), וגם אחרי חיבור SMTP חיצוני (Resend) התגלו עוד מכשולים (הגבלת "רק לכתובת שנרשמה", בלבול בין `localhost` לאתר האמיתי). לבקשת המשתמש - **ההתחברות הוסרה לגמרי**:

- אין `/login`, אין `/auth/callback`, אין `proxy.ts` (Middleware) שחוסם גישה.
- כל הבקשות (קריאה/כתיבה של `connector_configs`) עוברות עם ה-anon key ישירות, בלי session, בלי cookies - `lib/supabase/index.ts` (`getSupabase()`).
- **משרד יחיד קבוע** (`lib/default-office.ts`, `DEFAULT_OFFICE_ID = '00000000-...0001'`) - אין עוד `offices`/`office_members` פעילים; ה-migration `remove_auth_gate_use_shared_office` יוצרת את שורת המשרד הזו ומשנה את ה-RLS על `connector_configs` לפתוח (anon+authenticated, ללא תנאי).
- **החלטת אבטחה מכוונת שמצדיקה את זה**: הטבלה הזו לא מכילה Credentials, רק `type`/`url` של חיבור - אין שם שום דבר רגיש שדליפה שלו מהווה נזק אמיתי (Threat Model, סעיף Assets). זו לא הרחבה שהייתי עושה לטבלה עם נתונים אמיתיים.
- **מה זה אומר לעתיד**: ברגע שמתחילים לשמור נתוני לקוחות/Credentials אמיתיים (שלב 2+), התחברות אמיתית תחזור להיות הכרחית - רק הפעם עם SMTP מוגדר **לפני** ההשקה, לא כתיקון אחרי תקלה.
